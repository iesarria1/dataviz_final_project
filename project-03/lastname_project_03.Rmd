...
weather_tpa %>%
  ...
  geom_density( fill = "gray40", color = "black") +
  theme_minimal() +
  labs(
...
weather_tpa %>%
  mutate(
    precipitation = ifelse(precipitation == -99.99, NA, precipitation),
    month_name = factor(month.name[month], levels = month.name)
  ) %>%
  ggplot(aes(x = month_name, y = precipitation)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  theme_minimal() +
  labs(
    title = "Monthly Precipitation Distribution (Missing values removed)",
    x = "Month",
    y = "Precipitation"
  )
```
...
p <- weather_tpa %>%
  ...
  ggplot(aes(x = ave_temp, y = precipitation)) +
